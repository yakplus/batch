...
import com.likelion.backendplus4.yakplus.drug.infrastructure.persistence.dto.EmbeddingRequestText;
...
import com.likelion.backendplus4.yakplus.index.support.EmbeddingUtil.DrugEntityMapper;
...
public class KmBertEmbeddingLoadingAdapter implements EmbeddingLoadingPort {
    ...
    /**
     * 주어진 텍스트에 대해 외부 KM-BERT 임베딩 API를 호출하여 임베딩 벡터를 생성하고 반환합니다.
     *
    ...
    /**
     * 전달된 DrugVectorDto 리스트를 DrugKmBertEmbedEntity로 변환하여
    ...
    /**
     * Pageable 정보를 이용해 원시 약품 데이터와 KM-BERT 임베딩 데이터를 페이징 처리하여 조회하고,
    ...
    /**
     * KM-BERT 임베딩 API 호출을 위한 URI를 생성하여 반환합니다.
     *
     * @return Embedding API 호출에 사용할 URI
     * @author 함예정
     * @since 2025-04-25
     */
    ...
    /**
     * 지정된 URI에 텍스트를 포함한 요청을 전송하여 임베딩 벡터를 받아옵니다.
     *
     ...
     * @return Object[] 배열 리스트; [0]에는 DrugRawDataEntity, [1]에는 DrugKmBertEmbedEntity
     * @author 이해창
     * @since 2025-05-03
     */
    private List<Object[]> fetchRawAndEmbedPage(Pageable pageable) {
        return govDrugKmBertEmbedJpaRepository.findRawAndEmbed(pageable);
    }
     ...
     * Drug 도메인 객체로 변환합니다.
     *
     * @param pair Object 배열; index 0은 DrugRawDataEntity, index 1은 DrugKmBertEmbedEntity
    ...
    private Drug combineRawAndEmbed(Object[] pair) {
        ...
        DrugKmBertEmbedEntity embed = (DrugKmBertEmbedEntity) pair[1];
